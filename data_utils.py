...
class Dataset_train(Dataset):
    ...
    def __getitem__(self, index):
        ...
        X_noise =process_Rawboost_feature(X, fs, self.args, self.algo)
        X_pad= pad(X, self.cut)
        X_noise_pad= pad(X_noise, self.cut)
        x_inp= Tensor(X_pad)
        x_noise_inp= Tensor(X_noise_pad)
        target = self.labels[utt_id]
        return x_inp, x_noise_inp, target
...
class OOD_Dataset_eval(Dataset):
